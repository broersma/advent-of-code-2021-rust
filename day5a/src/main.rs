...
impl Line {
    ...
    fn inclusive_range(a: u32, b: u32) -> Box<dyn Iterator<Item=u32>> {
        if a <= b {
            Box::new(a..=b)
        } else {
            Line::inclusive_range(b, a)
        }
    }

    pub fn get_points(&self) -> Vec<(u32, u32)> {
        Line::inclusive_range(self.0, self.2).zip_longest(Line::inclusive_range(self.1, self.3)).map(|e| match e {
            Both(x,y) => (x,y),
            Left(x) => (x,self.1),
            Right(y) => (self.0, y)
        }).collect()
